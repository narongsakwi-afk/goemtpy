...
io.on('connection', (socket) => {
    ...
    socket.on('create_room', (roomData) => {
        ...
        // [FIXED] Use a consistent event for all waiting room updates
        socket.emit('update_waiting_room', rooms[roomId]);
    });
    ...
    socket.on('player_ready', (roomId) => {
        ...
        if (room) {
            ...
            // [FIXED] Notify EVERYONE in the room about the status change
            io.to(roomId).emit('update_waiting_room', room);
        }
    });
...
// ... The rest of the game logic functions (getNeighbors, etc.) remain unchanged ...
